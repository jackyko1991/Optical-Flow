...
public:
	...
	virtual bool loadCVImage(cv::Mat& cvImage);
	virtual bool getCVImage(cv::Mat& cvImage, ImageIO::ImageType imtype = ImageIO::standard) const;
#ifndef _MATLAB
...
template <class T>
bool Image<T>::loadCVImage(cv::Mat& cvImage)
{
	clear();
	if (ImageIO::loadCVImage(cvImage, pData, imWidth, imHeight, nChannels))
	{
		computeDimension();
...
template <class T>
bool Image<T>::getCVImage(cv::Mat& cvImage, ImageIO::ImageType imtype = ImageIO::standard) const
{
	if (IsDerivativeImage)
		imtype = ImageIO::derivative;
	ImageIO::getCVImage(cvImage, pData, imWidth, imHeight, nChannels, imtype);
	return true;
}
...
//------------------------------------------------------------------------------------------
// square root an image
//------------------------------------------------------------------------------------------
template <class T>
void Image<T>::Sqrt()
{
	for (int i = 0; i<nElements; i++)
		pData[i] = sqrt(pData[i]);
}
