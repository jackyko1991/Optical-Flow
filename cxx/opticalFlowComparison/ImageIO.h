...
public:
	template <class T>
	static bool loadCVImage(cv::Mat& cvImage, T*& pImagePlane, int& width, int& height, int& nchannels);
	template <class T>
	static bool getCVImage(cv::Mat& cvImage, const T* pImagePlane, int width, int height, int nchannels, ImageType imtype = standard);
	template <class T>
...
template <class T>
bool ImageIO::loadCVImage(cv::Mat& im, T*& pImagePlane, int& width, int& height, int& nchannels)
{
	...
	// check whether the type is float
	bool IsFloat = false;
...
template <class T>
bool ImageIO::getCVImage(cv::Mat& im, const T* pImagePlane, int width, int height, int nchannels, ImageType imtype)
{
	switch (nchannels) {
	case 1:
	...
	else
	{
		for (int i = 0; i<height; i++)
		{
			...
			for (int j = 0; j<im.step; j++)
			{
				...
				case standard:
					if (IsFloat)
						im.data[offset2 + j] = (double)(pImagePlane[offset1 + j]) * 255;
					else
